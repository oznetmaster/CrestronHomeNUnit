...
If release workflows must push metadata directly, a separate hardware-only branch rule can permit GitHub Actions and administrators to bypass that new rule while existing protections remain enforced. The Actions bypass applies to the integration generally, not one workflow; treat workflow changes as trusted infrastructure. The release preflight still checks the source before the version commit. This is source validation of tested Debug packages, not certification of independently rebuilt Release bytes. Do not activate required checks before a real App-reported run has passed and interruption/cleanup behavior has been reviewed.
