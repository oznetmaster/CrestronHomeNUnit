...
A later complete Wiser production-driver workflow passed local, processor and read-only live stages, updated the actual driver, deliberately failed a read-only installed-version assertion, then restored the exact reviewed prior package. It verified current configuration fingerprints, root and room-child identity/room scope, the restored loaded version and health. The original workflow remained failed, its temporary test instance was removed and its lease released; its owned archive was removed after the expected rollback outcome was reviewed. The verifier and exact-pair review are documented in [Wiser configuration rollback](https://github.com/oznetmaster/WiserHeatCrestronDriver/blob/master/docs/ConfigurationRollback.md). This does not establish compatibility for another driver or an unreviewed pair.
