...
**Current status:** the CLI implements the gated workflow and restores DevTools from NuGet. Representative Entity V2 and explicitly opted-in V1 update/reboot workflows passed hardware validation. V1 initial-install/removal was also exercised, with read-only resumed startup verification after a timeout. These results do not establish all driver behaviors. The stable .NET 10 adapter is available as CrestronHomeNUnit.TestAdapter; see [setup and validation](VisualStudioTestExplorer.md).
...
Installed-driver checks read properties and validate expected values/ranges by default. From tooling 1.4.0, workflows also support opt-in `deployedControls`: absolute commands, independent physical observations and verified restoration under the processor reservation. See [the control contract and private plan](InstalledDriverControls.md). Hardware validation covers selected command pairs only. Every consuming driver needs its own validated probe and restoration design; never assume a device's initial value.
...
For standard projects whose manifest sits beside the project and has the same basename as either the project or the output package, the workflow reconciles the fourth-component Debug counter with the highest same-model catalogue version while holding its processor lease. This includes drivers renamed for Crestron submission. If both filenames exist, set the package plan's `manifestPath` to an existing absolute path to choose explicitly. The manifest must belong to a declared source root. The normal project build still increments that counter, and the workflow verifies the resulting driver identity, release components and newer revision before upload. Manifest formatting, UTF-8 BOM and unrelated fields are preserved. A newer release already on the processor, an unreadable matching version or an exhausted revision stops the build for deliberate correction.

Custom manifest layouts can supply `manifestPath` to use the same reconciliation. Without it, unrecognized layouts retain the existing collision checks and must manage their Debug counter explicitly. Keep private CI counters when using disposable checkouts: they preserve allocated revisions after catalogue packages are removed. The workflow rechecks for equal or newer matching catalogue versions immediately before deployment; a conflicting manual deployment stops that run.
...
Representative driver workflows have exercised actual-driver updates, read-only installed checks, initial configuration and explicitly authorized V1 reboot handling. Separate control runs have verified independent device observation and state restoration. Per-project inventories and results belong in the consuming repositories. Passing code inside a processor test package does not establish that the production driver was installed or tested.

CP4-R / Home 4.11.322 validation additionally covered a temporary Entity V2 test host: installation, update, current-instance reuse, targeted reload, authorized reboot, reconnection, lease verification and removal. Its 116 tests passed before and after reboot. All seven pre-existing driver instances retained their identities, room assignments and versions and were Loaded. This was test-host lifecycle validation; no existing production driver was updated on the CP4-R.
...
- Provide driver-specific independent observations and restoration for each control route. Read-only devices and devices without observable state require an appropriate separate test design.
- Validate each V1 lifecycle and shared-instance scope before enabling it in that driver's workflow. The tested initial-install path required read-only resumed startup verification; commissioning and reboot were not repeated.
...
- Supply a compatibility review and configuration verifier for each exact driver rollback pair. A verified pair does not establish safety for another pair; V1 and initial-install rollback remain unsupported.
...
The implementation has simulated regression coverage for authorization, lost update responses preventing reboot, no duplicate reboot, failed reconnection, changed driver identity, saved evidence and retained lease ownership. The complete V1 driver update cycle has now passed on the development MC4-R; V1 driver initial installation and removal have also been verified on MC4-R, including preservation of the existing shared-code instance; initial startup verification was resumed after a timeout. Existing successful Entity V2 workflow evidence remains valid for the default reboot-free path.
...
{
  "steps": [
    { "id": "Connection", "values": { "_Host_": "192.0.2.10", "CredentialSettingId": "REPLACE_LOCALLY" } },
    { "id": "OptionsStepId", "values": { "OptionSettingId": "EXAMPLE_VALUE" } }
  ]
}
```
