...
public static class ProcessorAuthentication
	{
	public static Task<ProcessorConnection> AuthenticateAsync (string host, string user, string password, string expectedProcessorId, Func<string, string> getKnownKey, Action<string, string> rememberKey, bool trustUnknownHost = false) => Task.Run (() =>
		{
			...
			string known = getKnownKey ("fingerprint:" + host.ToLowerInvariant ());
			if (known.Length == 0 && expectedProcessorId.Length != 0)
				known = getKnownKey ("fingerprint-id:" + expectedProcessorId);
			string fingerprint = "";
			...
			client.HostKeyReceived += (_, args) =>
				{
					...
					args.CanTrust = (known.Length == 0 && trustUnknownHost) || known == fingerprint;
				};
			client.Connect ();
			...
			rememberKey ("fingerprint:" + host.ToLowerInvariant (), fingerprint);
			rememberKey ("fingerprint-id:" + identity.Id, fingerprint);
			return identity;
		});
	}