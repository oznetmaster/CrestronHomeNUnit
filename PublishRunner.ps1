...
param(
    ...
    [string] $OutputDirectory = (Join-Path $PSScriptRoot 'artifacts\runner')
)
...
& dotnet publish (Join-Path $PSScriptRoot 'CrestronHomeNUnit.Runner\CrestronHomeNUnit.Runner.csproj') `
...
if ($LASTEXITCODE -ne 0) { throw 'Windows runner publish failed.' }
# Preserve the notices shipped with the exact runtime packs selected for this release.
$assets = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'CrestronHomeNUnit.Runner\obj\project.assets.json') -Raw | ConvertFrom-Json -AsHashtable
...
$archive = Join-Path $outputRoot ("CrestronHomeNUnit.Runner-$Runtime.zip")
...
Write-Host "Self-contained Windows runner: $archive"
