...
function Normalize-TestName([string]$Name) {
    ...
    for($index=0;$index -lt $Name.Length;$index++) {
        ...
        if($quote -eq [char]0 -and $Name.Substring($index).StartsWith('_Stripped.System.',[StringComparison]::Ordinal) -and
            ($index -eq 0 -or $Name[$index-1] -notmatch '[\w.]')) {
            [void]$result.Append('System.')
            $index+='_Stripped.System.'.Length-1
...
function Read-TestTree([string]$Path, [switch]$AdapterDump) {
    ...
    foreach ($case in $cases) {
        ...
        [pscustomobject]@{ Name=(Normalize-TestName $case.GetAttribute('fullname')); Live=('Live' -in $categories); LiveControl=('LiveControl' -in $categories); Processor=('Processor' -in $categories); Result=$case.GetAttribute('result'); Label=$case.GetAttribute('label'); Reason=$case.SelectSingleNode('reason/message') }
    }
}
...
function Find-DiscoveredCase($Inventory, [string]$Identity) {
    $matches = @($Inventory | Where-Object Name -CEQ $Identity)
    if (!$matches.Count) { throw "Unknown result: $Identity" }
    # NUnit truncates long argument displays, so distinct cases can share a name.
    # Preserve their multiplicity, but never use an ambiguous name to allow a skip.
    $kinds = @($matches | ForEach-Object { "$($_.Live)/$($_.LiveControl)/$($_.Processor)" } | Sort-Object -Unique)
    if ($kinds.Count -ne 1) { throw "Conflicting categories for result: $Identity" }
    return $matches[0]
}
...
if ($Stage -eq 'Desktop') {
    ...
    dotnet test $projectPath -c $Configuration -f $Framework --list-tests --filter 'TestCategory=Processor|TestCategory!=Processor' -p:DeployAfterBuild=false -- NUnit.DumpXmlTestDiscovery=true
    ...
    if ($DiscoveryOnly) {
        $inventory | Select-Object Name,Live,LiveControl,Processor | ConvertTo-Json -Depth 4 | Set-Content "$evidence/inventory.json" -Encoding utf8
        Write-Host "Verified discovery of $($inventory.Count) tests without executing them."
        return
    }
    $filter = if ($ExcludeProcessor) { 'TestCategory!=Live&TestCategory!=Processor' } else { 'TestCategory!=Live' }
    dotnet test $projectPath -c $Configuration -f $Framework --no-build --filter $filter --logger 'trx;LogFileName=tests.trx' --results-directory $evidence
    ...
    foreach ($result in $trx.TestRun.Results.UnitTestResult) {
        $execution = $result.GetAttribute('executionId')
        if (!$execution -or !$executions.Add($execution)) { throw 'Missing or duplicated test execution record.' }
        $definition = $definitions[$result.GetAttribute('testId')]
        if ($null -eq $definition) { throw 'Result has no test definition.' }
        $identity = Normalize-TestName ($definition.TestMethod.GetAttribute('className') + '.' + $definition.GetAttribute('name'))
        $match = Find-DiscoveredCase $inventory $identity
        $match.Result = $result.GetAttribute('outcome')
        $match.Reason = $result.SelectSingleNode("*[local-name()='Output']/*[local-name()='ErrorInfo']/*[local-name()='Message']")
        Assert-TestOutcome $match $AllowProcessorSkips.IsPresent
        $observed += $identity
    }
    ...
    foreach ($tree in $trees) {
        ...
        if ($category -notin @('unit','processor','live','live-read','live-control')) { throw 'Unknown suite category.' }
        $expected = @($inventory | Where-Object { $candidate = $_; switch ($category) { 'unit' { !$candidate.Live -and !$candidate.Processor } 'processor' { !$candidate.Live -and $candidate.Processor } 'live' { $candidate.Live } 'live-read' { $candidate.Live -and !$candidate.LiveControl } 'live-control' { $candidate.Live -and $candidate.LiveControl } } } | ForEach-Object Name)
    ...
    foreach ($run in $runs) {
        ...
        Assert-SameTests @($SuiteCategories.Keys | Where-Object { $SuiteCategories[$_] -notlike 'live*' }) @($files | ForEach-Object { $_.Directory.Name }) 'Executed package suites'
        $executed = @()
