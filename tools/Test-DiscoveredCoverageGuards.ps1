...
$first=[pscustomobject]@{Name='Fixture.Array(...)';Live=$false;LiveControl=$false;Processor=$false}
$second=[pscustomobject]@{Name=$first.Name;Live=$false;LiveControl=$false;Processor=$false}
$matched=Find-DiscoveredCase @($first,$second) $first.Name
if (![object]::ReferenceEquals($matched,$first)) { throw 'Duplicate display names were not resolved consistently.' }
$checks++
Assert-SameTests @($first.Name,$second.Name) @($first.Name,$second.Name) 'Duplicate display names'
$checks++
Reject { Assert-SameTests @($first.Name,$second.Name) @($first.Name) 'Missing duplicate display case' }
$second.Processor=$true
Reject { Find-DiscoveredCase @($first,$second) $first.Name }
Reject { Find-DiscoveredCase @($first) 'Unknown.Test' }
foreach($pair in @(
    @('Fixture.Type(_Stripped.System.String)','Fixture.Type(System.String)'),
    @('Fixture.Type("_Stripped.System.String",_Stripped.System.Int32)','Fixture.Type("_Stripped.System.String",System.Int32)'),
    @('Fixture.Type("a\"_Stripped.System.String",_Stripped.System.Int32)','Fixture.Type("a\"_Stripped.System.String",System.Int32)'),
    @('Fixture.Other_Stripped.System.String','Fixture.Other_Stripped.System.String')
)) {
    if((Normalize-TestName $pair[0]) -cne $pair[1]) { throw 'Shim-name normalization changed test data or missed a type.' }
    $checks++
}
